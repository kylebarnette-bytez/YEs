...
public class BoardPanel extends JPanel {
    ...
    /** GUI-level move history (used for save/load & rebuilding history panel). */
    private final Stack<MoveRecord> moveHistory = new Stack<>();
    ...
    public void handleSquareClick(SquarePanel clicked) {
        ...
        // --- Record backend move for true undo (before mutating board) ---
        Piece movingCopy = backendBoard.clonePiece(moving, moving.getPosition());
        Piece capturedCopy = (destBefore == null)
                ? null
                : backendBoard.clonePiece(destBefore, destBefore.getPosition());
        ...
        // --- Build move notation & MoveRecord for save/load + historyPanel ---
        String movingKey = pieceToKey(moving);
        String fromStr = "(" + from.getRow() + "," + from.getCol() + ")";
        String toStr   = "(" + to.getRow() + "," + to.getCol() + ")";
        String moveText = movingKey + ": " + fromStr + " → " + toStr;

        String capturedKey = (destBefore != null) ? pieceToKey(destBefore) : null;
        String capturedBy  = (destBefore != null)
        ...
        if (historyPanel != null) {
            ...
            if (capturedKey != null && capturedBy != null) {
                historyPanel.addCapturedPiece(capturedBy, capturedKey);
            }
        }
    ...
    /**
     * Saves the FULL backend game state (board, turn, move history text, and captured pieces)
     * to a file chosen by the user.
     */
    public void saveGame() {
        JFileChooser fileChooser = new JFileChooser();
        ...
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(file))) {
            ...
            // Save GUI move history (text + capture info)
            for (MoveRecord m : moveHistory) {
                state.moveHistory.add(new GameState.MoveData(
                        m.fromRow,
                        m.fromCol,
    ...
    /**
     * Loads a previously saved game from file and fully restores backend state,
     * GUI state, move history, captured pieces, and turn state.
     */
    public void loadGame() {
        JFileChooser fileChooser = new JFileChooser();
        int option = fileChooser.showOpenDialog(this);
        ...
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(file))) {
            ...
            // ----------------------------------------------------
            backendBoard = new Board();
            backendBoard.clearAllPieces();   // helper in Board.java
            ...
            // ----------------------------------------------------
            // 3) RESTORE MOVE HISTORY + CAPTURED LISTS (GUI + internal stack)
            // ----------------------------------------------------
            moveHistory.clear();
            if (historyPanel != null) {
                historyPanel.reset();
            ...
            for (GameState.MoveData md : state.moveHistory) {
                ...
                if (historyPanel != null && md.notation != null) {
                    historyPanel.addMove(md.notation);
                    if (md.capturedPiece != null && md.capturedBy != null) {
                        historyPanel.addCapturedPiece(md.capturedBy, md.capturedPiece);
                    }
                }
            }
...
// For each saved move, re-simulate it to rebuild undo history
    ...
    /**
     * Undoes the most recent move, restoring the backend board state,
     * GUI board, move history, and captured lists.
     */
    public void undoLastMove() {
        // 1) We can only undo if backend has moves
        if (backendHistory.isEmpty()) {
        ...
        // Also pop GUI MoveRecord if present
        ...
        // 4) Rebuild the history panel from remaining moveHistory
        if (historyPanel != null) {
            historyPanel.reset();
            for (MoveRecord r : moveHistory) {
                if (r.notation != null) {
                    historyPanel.addMove(r.notation);
                }
                if (r.capturedPiece != null && r.capturedBy != null) {
                    historyPanel.addCapturedPiece(r.capturedBy, r.capturedPiece);
                }
            }
        }
    ...
    /**
     * Represents a record of a single chess move, used for save/load
     * and for rebuilding the GameHistoryPanel.
     */
    private static class MoveRecord {
        private final SquarePanel from, to;
        private final String capturedPiece;
        ...
        private final String capturedBy; // "White" or "Black" if capture occurred

        MoveRecord(SquarePanel from,
                   SquarePanel to,
                   String notation,
                   String capturedPiece,
                   String capturedBy) {
    ...
    /** Serializable container for FULL backend save data + move history. */
    private static class GameState implements Serializable {
        PieceData[][] board = new PieceData[BOARD_SIZE][BOARD_SIZE];
        boolean whiteTurn;
        List<MoveData> moveHistory = new ArrayList<>();
        ...
        /** Describes one move for save/load (board coords + history info). */
        static class MoveData implements Serializable {
            int fromRow, fromCol, toRow, toCol;
            String capturedPiece; // e.g., "BLACK_PAWN" or null
            String capturedBy;    // "White" or "Black" or null
            String notation;      // e.g., "WHITE_PAWN: (6,4) → (4,4)"

            MoveData(int fromRow,
                     int fromCol,
                     int toRow,
                     int toCol,
                     String capturedPiece,
